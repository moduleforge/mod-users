...
- ~~**Extract `UserAccountsService`**~~ (architect M4, resolved Phase F). `UserAccountService` now lives in `api/internal/service/user_accounts.go`. The handler is a thin parse-call-render layer. `NaturalPersonService.CreateInTx` enables atomic entity+user_account creation in one transaction — the two-phase tx workaround is gone.
