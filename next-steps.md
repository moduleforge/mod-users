...
- **Extract `UserAccountsService` and `AppsService`** (architect M4). Standard service-method shape is currently implemented at the HTTP-handler layer (mixed with HTTP plumbing). Should be extracted into dedicated service types like `tags-module`/`contacts-module` do. Substantial refactor. Symptom: `NaturalPerson.Create` two-phase tx workaround in users-module.
