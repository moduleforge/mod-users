...
**Anonymous.** `POST /v1/auth/anonymous` creates a `user_account` with a NULL email (no credentials required) and a corresponding `anon_tokens` row. The response includes a signed JWT (with `is_anonymous: true` claim) and a raw session token. The session token can be presented on a subsequent visit — keyed by `device_id` — to recover the same anonymous identity across sessions. When the user later provides an email address (via `PUT /v1/self`), the service upgrades the account: the `email` column is set, `is_anonymous` becomes false, and all `anon_tokens` rows for that account are deleted. The `login`, `email-code`, and `password-reset` handlers are documented as guarding against anonymous accounts and returning `400 anonymous_account` if called with an anonymous JWT — a flat `{"error": "anonymous_account", ...}` envelope rather than the module's canonical nested `{error:{code,message[,details]}}` shape used elsewhere in the API. This behavior could not be confirmed against the current `api/internal` Go source: `anonymous_account` does not appear anywhere in that tree, and the `login`/`email-code`/`password-reset` handlers show no `IsAnonymous`-style guard matching the description. It may itself be stale documentation rather than an intentional, deferred non-conformance; see follow-up `nnfn` for a dedicated accuracy pass before drawing that conclusion.
