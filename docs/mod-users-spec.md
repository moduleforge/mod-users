...
When the anonymous user later provides an email address (via `PUT /v1/self`), the account is upgraded: the `email` column is set to the provided value, `is_anonymous` becomes `false`, and all `anon_tokens` rows for that account are deleted. Subsequent auth flows (login, email-code, password-reset) that require an email address are documented as guarding against anonymous accounts and returning `400` with code `anonymous_account` if called with an anonymous JWT — a flat, non-nested error body that would not conform to the module's canonical `{error:{code,message[,details]}}` envelope described under [General features](#general-features). This behavior could not be confirmed against current `api/internal` source (`anonymous_account` does not appear anywhere in the Go tree, and the relevant handlers show no matching guard logic), so it may itself be stale documentation rather than an actual, intentionally-deferred non-conformance; see follow-up `nnfn` for a dedicated accuracy pass.
...
- **All errors return a structured payload**: a top-level `error` object with a machine-readable `code` (drawn from a reserved, closed vocabulary — `unauthenticated`, `forbidden`, `not_found`, `invalid_input`, `conflict`, `internal_error`), a human-readable `message`, and an optional `details` array of `{field, code, message}` entries carrying finer-grained, module-namespaced distinctions (e.g. `users.email_taken`) — `{"error": {"code": ..., "message": ..., "details": [...]}}`. See [`api/openapi.yaml`](../api/openapi.yaml)'s `Error`/`FieldError` schemas for the authoritative shape. A small number of legacy endpoints are documented elsewhere as emitting a flat, non-nested error body instead (e.g. the `400 anonymous_account` case noted in [use case 14](#14-create-an-anonymous-account-and-optionally-upgrade-it) and in [Security requirements](#security-requirements)); that documented behavior could not be confirmed against current source and may itself be stale — see follow-up `nnfn`.
...
- **Authentication:** All non-public endpoints require a valid signed JWT passed as a bearer token. JWTs are issued by this module and verified on every request. JWTs issued for anonymous accounts carry an `is_anonymous: true` claim; email-dependent auth operations (`login`, `email-code`, `password-reset`) are documented as rejecting these tokens with `400 anonymous_account` — a flat-envelope response that would not match the module's canonical nested error shape (see [General features](#general-features)). This behavior could not be confirmed against current `api/internal` source and may itself be stale documentation rather than an intentional, deferred non-conformance; see follow-up `nnfn` for a dedicated accuracy pass.
