...
The module owns eight Postgres tables. All are managed by goose migrations under `model/migrations/`; Go query code is generated by sqlc.

| Table | Purpose |
|---|---|
| `apps` | Application tenants (name, slug, soft-delete). |
| `user_accounts` | One row per user. References `legal_entities(entity_id)` (core-module); holds the canonical email, email-verified timestamp, and default-app FK. |
...
- **Authentication:** All non-public endpoints require a valid signed JWT passed as a bearer token. JWTs are issued by this module and verified on every request.
